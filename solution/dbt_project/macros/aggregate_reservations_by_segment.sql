            ...
            SELECT 
                ...
                ROUND(
                    (COUNT(*)::numeric / NULLIF(
                        SUM(COUNT(*)) OVER (PARTITION BY {{ segment_type }}), 0
                    )) * 100, 2
                ) AS relative_pct_num_of_reservations_by_{{ segment_type }},
                SUM(number_of_nights) as number_of_nights,
                ROUND(
                    (SUM(number_of_nights) / NULLIF(
                        SUM(SUM(number_of_nights)) OVER (PARTITION BY {{ segment_type }}), 0
                    )) * 100, 2
                ) AS relative_pct_num_of_nights_by_{{ segment_type }},
                ROUND(SUM(total_night_cost)::numeric, 2) AS total_night_cost,
                ROUND(
                    (SUM(total_night_cost)::numeric / NULLIF(
                        SUM(SUM(total_night_cost)::numeric) OVER (PARTITION BY {{ segment_type }}), 0
                    )) * 100, 2
